...
export class NotificationGroupingService {
  ...
  private shouldGroupNotifications(newNotification: any, existingNotification: Notification): boolean {
    ...
    return timeDiff < groupingTimeWindow &&
      newNotification.type === existingNotification.type &&
      newNotification.actorId === existingNotification.actorId;
  }

  private async updateNotificationGroup(existingNotificationId: string, newNotificationData: any): Promise<Notification> {
    ...
    this.logger.log(`Grouped notification ${existingNotificationId} with new notification ${newNotificationData.type}`);
